...
vidcap = cv2.VideoCapture("./shahar_walk.avi")
...
while vidcap.isOpened():
    ...
    if not flag:
        break
    frame_count += 1
    cv2.imwrite("./frames/%d.png" % frame_count, image)
    frames.append(image)
...
for f in tqdm(range(frame_count)):
    ...
    cv2.imwrite("./abs_sub/%d.png" % (f+1), abs_sub)
    ...
    cv2.imwrite("./thresh/%d.png" % (f+1), thresh)
    #Find the ROI of the given thresholded image.
    thresh_img = cv2.imread("./thresh/%d.png" % (f+1),0)
...
video = cv2.VideoWriter('./median_video.avi', fourcc, 24, (vid_width,vid_height))
for i in tqdm(range(frame_count)):
    video.write(cv2.imread("./circled/%d.png" % (i+1)))
